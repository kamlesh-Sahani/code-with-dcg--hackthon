...
export const addQuestionToSet = async (req: Request, res: Response) => {
  try {
...
export const getAllQuestionSets = async (req: Request, res: Response) => {
  try {
    const questionSets = await QuestionSetModel.find({ isDeleted: false }).sort(
      { createdAt: -1 }
    );
...
export const deleteQuestionFromSet = async (req: Request, res: Response) => {
  try {
    ...
    const questionIndex = questionSet.questions.findIndex(
